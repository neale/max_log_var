...
writer = SummaryWriter(log_dir='runs/ant/run_var_record2')
...
def infra_config():
    ...
    omp_num_threads = 16                             # for high CPU count machines
...
def exploration():
    ...
    #utility_measure = 'traj_stdev'                   # measure for calculating exploration utility of a particular (state, action). 'cp_stdev', 'renyi_div'
    # utility_measure = 'cp_stdev'                   # measure for calculating exploration utility of a particular (state, action). 'cp_stdev', 'renyi_div'
...
def transition_novelty(state, action, next_state, model, renyi_decay):
    ...
    measure2 = CompoundProbabilityStdevUtilityMeasure()
    #measure = PredictionErrorUtilityMeasure()
    v = measure(state, action, next_state, mu, var, model)
    v2 = measure2(state, action, next_state, mu, var, model)
    return v.item(), v2.item()
...
def evaluate_utility(buffer, exploring_model_epochs, model_train_freq, n_eval_episodes, _log, _run):
    ...
    measure = get_utility_measure(utility_measure='cp_stdev', utility_action_norm_penalty=0)
...
def do_max_exploration(seed, action_noise_stdev, n_exploration_steps, n_warm_up_steps, model_train_freq, exploring_model_epochs,
    ...
    for step_num in range(1, n_exploration_steps + 1):
        ...
        if step_num > n_warm_up_steps:
            v1, v2 = transition_novelty(state, action, next_state, model=model)
            writer.add_scalar("experience_novelty", v1, step_num)
            writer.add_scalar("experience_novelty_var", v2, step_num)
